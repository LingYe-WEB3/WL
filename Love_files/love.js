(function (window) {
  ...
  // 我添加的代码：
  //   var canvas = document.getElementById("canvas"); // Replace 'yourCanvasId' with your canvas element's ID
  ...
  //     // Iterate through the tree's seeds (leaves) and check if the click event is within a seed's boundaries
  //     for (var i = 0; i < tree.seeds.length; i++) {
  //       var seed = tree.seeds[i];
  //       if (isPointInSeed(x, y, seed)) {
  //         // Handle the click on the seed (leaf) here
  //         // You can do something like displaying information or performing an action
  //         alert("Leaf " + (i + 1) + " clicked!");
  //       }
  //     }
  //   }

  //   function isPointInSeed(x, y, seed) {
  //     var seedX = seed.cirle.point.x;
  //     var seedY = seed.cirle.point.y;
  //     var radius = seed.cirle.radius * seed.cirle.scale;

  //     // Check if the click coordinates (x, y) are within the circular boundaries of the seed
  //     return (
  //       x >= seedX - radius &&
  //       x <= seedX + radius &&
  //       y >= seedY - radius &&
